...
infolayers <-  layer(panel.scaleArrow(x = 0.99, digits = 1, col = "grey", srt = 90, cex = 0.7)) +  layer(lim <- current.panel.limits(), panel.text(lim$x[1], lim$y[1], round(lim$y[1],1), font = 2,   cex = 0.7, adj = c(-0.5,-0.5), col = "#9FC8DC"))
...
horizonplot(dat, layout = c(1,12), origin = 0,scales = list(y = list(relation = "same")),  colorkey = TRUE, colorkey.digits = 1) +infolayers
...
xyplot(EuStockMarkets, scales = list(y = "same"), panel = function(x, y, ...) {  col <-
...
horizonplot(EuStockMarkets, colorkey = TRUE,origin = 4000, horizonscale = 1000) +  infolayers
